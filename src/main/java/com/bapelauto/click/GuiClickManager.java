...
public class GuiClickManager {
    ...
    public void captureTarget(Minecraft client, long defaultDelay) {
        ...
        if (client.screen instanceof AbstractContainerScreen) {
            ...
            if (slot != null) {
                ...
                if (client.player != null) {
                    client.player.sendSystemMessage(Component.literal("§e[Capture] Slot: " + slot.index), true);
                }
            }
        }
        ...
        if (newTarget == null) {
            ...
            if (client.player != null) {
                client.player.sendSystemMessage(Component.literal("§e[Capture] Point: " + (int)mouseX + ", " + (int)mouseY), true);
            }
        }
        ...
        if (macroRecorder.isRecording()) {
            ...
            if (client.player != null) {
                client.player.sendSystemMessage(Component.literal("§a[Recording] Step " + macroRecorder.getActionCount()), true);
            }
        } else {
            // Normal capture mode
            ...
            if (client.player != null) {
                client.player.sendSystemMessage(Component.literal("§6[Capture] Total: " + capturedTargets.size()), true);
    ...
    public void stopRecording(Minecraft client) {
        ...
        if (macroRecorder.getActionCount() > 0) {
            ...
            if (client.player != null) {
                client.player.sendSystemMessage(Component.literal("§e[Macro] Ready to replay - Press [=] to start"), true);
            }
        }
    }
    ...
    public void toggle(Minecraft client) {
        if (capturedTargets.isEmpty()) {
            if (client.player != null) {
                client.player.sendSystemMessage(Component.literal("§c[Click] No targets! Press [-] to capture"), false);
            }
            isActive = false;
            return;
        }
        ...
        if (client.player != null) {
            ...
            client.player.sendSystemMessage(Component.literal("§6[Click] " + status + mode + pattern), true);
        }
    }
